...
document.onload = (function(d3, saveAs, Blob, undefined) {
  ...
  var Graphmaker = function(svg, nodes, links) {
    ...
    thisGraph.shapeNum = {"circle": 1, "rectangle": 1, "diamond": 1, "ellipse": 1, "star": 1,
                          "noBorder": 1};
    thisGraph.state = {
      selectedNode: null,
      selectedEdge: null,
    ...
    // Set up notes, i.e., tool tips:
    thisGraph.tip = d3.tip()
                      .attr("class", "d3-tip")
                      .offset([-10, 0])
                      ...
                      .html(function (d) {
                        ...
                        return  d.note ? d.note : null; 
                      });
    d3.select("#mainSVG").call(thisGraph.tip);
  ...
  Graphmaker.prototype.consts =  {
    ...
    defaultFontSize: 12 // Don't forget to set this value in css file
  };
  ...
  Graphmaker.prototype.prepareToolbox = function() {
    ...
    var selectShape = function(selectedElt, shapeSelection) {
      ...
      d3.select("#ellipseSelection").style("stroke", thisGraph.consts.unselectedStyleColor)
        .classed("sel", false).classed("unsel", true)
      d3.select("#starSelection").style("stroke", thisGraph.consts.unselectedStyleColor)
        .classed("sel", false).classed("unsel", true)
    ...
    d3.select("#shapeSelectionSvg").append("rect")
      ...
      .attr("height", thisGraph.minRectSide -5)
      .attr("x", thisGraph.sssw / 2.0  - thisGraph.minRectSide + 17)
      .attr("y", thisGraph.consts.ssRectangleY)
    ...
    var starCtrX = thisGraph.consts.minEllipseRx * 2;
    var starCtrY = thisGraph.consts.ssNoBorderXformY + thisGraph.minRectSide * 0.7 + 18;
    d3.select("#shapeSelectionSvg").append("polygon")
      .attr("id", "starSelection")
      .classed("shapeSelection", "true")
      .attr("x", starCtrX)
      .attr("y", starCtrY)
      .attr("points", thisGraph.calculateStarPoints(starCtrX, starCtrY, 5, 30, 15))
      .classed("unsel", true).classed("sel", false)
      .style("stroke", thisGraph.consts.unselectedStyleColor)
      .style("stroke-width", 2)
      .on("click", function(d) {  selectShape(d3.select(this), "star"); });
    ...
    d3.select("#shapeSelectionSvg").append("text")
      ...
      .attr("y", thisGraph.consts.ssNoBorderXformY + thisGraph.minRectSide * 0.7 + 58)
      .classed("unsel", true).classed("sel", false)
  ...
  Graphmaker.prototype.calculateStarPoints = function(ctrX, ctrY, arms, outerRadius, innerRadius) {
    ...
    var rotation = Math.PI / (10 / 3.0); // 1st point up rather than on the +x axis
 
    for (var i = 0; i < 2 * arms; i++) { 
      var r = (i % 2) ? innerRadius : outerRadius; // Alternate outer and inner radii
      var currX = ctrX + Math.cos(i * angle + rotation) * r;
      var currY = ctrY + Math.sin(i * angle + rotation) * r;
  ...
  // Returns an array of hex color values computed by looping around the hsl hue "perimeter."
  Graphmaker.prototype.colorChoicesComputed = function() {
    var nColors = 12;
    var colorArray = [];
  ...
  Graphmaker.prototype.svgMouseUp = function() {
    ...
    if (state.justScaleTransGraph) { // Dragged not clicked
      ...
      var d = {id: thisGraph.idct,
               name: thisGraph.consts.defaultShapeText[thisGraph.shapeSelected] + " "
                   + thisGraph.shapeNum[thisGraph.shapeSelected]++,
               x: xycoords[0],
               y: xycoords[1],
               color: thisGraph.clr,
  ...
  Graphmaker.prototype.showSystemSupportMap = function() {
    var thisGraph = this;
    if (!thisGraph.SSMCenter) {
      thisGraph.SSMCenter = {"x": d3.select("#graph").node().clientWidth / 2,
                             "y": d3.select("#graph").node().clientHeight / 2};
    }
    var ssmCenter = thisGraph.SSMCenter;
    d3.select(".ssmGroup")
      ...
      .attr("y", function(d, i) {
        ...
        switch(i) {
          ...
          case 4:
            ...
            break;
          default:
            console.log("showSystemSupportMap(): unknown ring number");
            break;
        }
        return ssmCenter.y - d.radius + offset;
      });
    d3.select("#optionsOption0").text(thisGraph.consts.ssmHideText)
      .datum({"name": thisGraph.consts.ssmHideText});
  };
  ...
  Graphmaker.prototype.hideSystemSupportMap = function() {
    ...
    d3.select("#optionsOption0").text("Show system support rings")
  ...
  // Return dimensions of bounding box for all visible objects (plus a little extra).
  Graphmaker.prototype.getGraphExtent = function(shapes) {
    ...
    shapes.each(function(d) {
      if (d3.select(this).style("display") !== "none") { // Don't include hidden objects
        ...
        var x = (d.x) ? d.x : parseFloat(d3.select(this).attr("cx")); // ssmCircles: no x, y
        var y = (d.y) ? d.y : parseFloat(d3.select(this).attr("cy"));
  ...
  Graphmaker.prototype.exportGraphAsImage = function() {  
    var shapes = d3.selectAll(".ssmGroup circle, g.shapeG circle, g.shapeG rect, g.shapeG ellipse, "
      + "g.shapeG path, g.shapeG polygon");
    ...
    image.onload = function() {
      ...
      a.download = "SystemSupportGraph.png";
      a.href = canvasData;
      a.click();
    };
    image.onerror = function() { console.log("Export to image failed"); };
    ...
    // Reset dimensions and attributes for normal interactive behavior:
  ...
  Graphmaker.prototype.createOptionsMenu = function() {
    var thisGraph = this;
    var items = [{"name": "Show system support rings"},
  ...
  /** MAIN SVG **/
  d3.select(settings.appendElSpec).append("div")
    .attr("id", "mainSVGDiv");
